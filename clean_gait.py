...
# ─────────────────────────── CONFIG ────────────────────────────
INPUT_CSV   = Path("../7_W00_140721_15.10_TV1DLC_Resnet50_W00-DLCApr30shuffle1_snapshot_best-90_filtered-1778216802446.csv")
OUT_DIR     = Path(".")
...
def plot_likelihood_overview(df_raw: pd.DataFrame, parts: list, thresh: dict, out_dir: Path):
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "01_likelihood_filtering.png", dpi=150)
    plt.close(fig)
...
def plot_motion_score(df_raw: pd.DataFrame, smooth_score: pd.Series,
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "02_motion_bouts.png", dpi=150)
    plt.close(fig)
...
def plot_xy_overview(df_raw: pd.DataFrame, df_clean: pd.DataFrame,
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "03_xy_raw_vs_clean.png", dpi=150)
    plt.close(fig)
...
def plot_step_quality(quality: dict, bout_meta: pd.DataFrame, out_dir: Path):
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "04_step_quality.png", dpi=150)
    plt.close(fig)
...
def plot_sample_bouts(df_clean: pd.DataFrame, parts: list, quality: dict, out_dir: Path):
    ...
    plt.tight_layout()
    fig.savefig(out_dir / "05_individual_bouts.png", dpi=150)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    df_clean.to_csv(OUT_DIR / "cleaned_gait_data.csv", index=False)
    ...
    bout_meta.to_csv(OUT_DIR / "bout_metadata.csv", index=False)
    ...
    print(f"\n  Output files:")
    print(f"    cleaned_gait_data.csv  — per-frame cleaned coords + bout_id + step_quality")
    print(f"    bout_metadata.csv      — bout-level summary")
    print(f"    01_likelihood_filtering.png")
    print(f"    02_motion_bouts.png")
    print(f"    03_xy_raw_vs_clean.png")
    print(f"    04_step_quality.png")
    print(f"    05_individual_bouts.png")
    print()
